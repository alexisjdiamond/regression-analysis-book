...
\subsection{A caution about first differences specifically}

One clarification prevents an overcorrection. Suppose that instead of a predicted \emph{level} we want a first difference: how much higher is the predicted stopping distance at \(\text{speed}=20\) than at \(\text{speed}=15\)? In the simple model with an intercept,
\[
...
But this survives only because the model is so simple. Add a quadratic term and the same first difference becomes
\[
...
\item[First difference] The change in a predicted or fitted quantity produced by moving a predictor from one value to another, such as \(\widehat{Y}(x_2)-\widehat{Y}(x_1)\). Its uncertainty generally involves several estimated coefficients and therefore their covariances; it can be obtained by rescaling a single coefficient's confidence interval only when the difference depends on exactly one coefficient.\index{first difference}
