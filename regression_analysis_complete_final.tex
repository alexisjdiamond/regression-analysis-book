...
\subsection{A warning about the word ``expect''}
\index{expected value!meaning of the term}\index{expected value!need not be observable}

The word \emph{expect} is doing a great deal of work too, and it is worth pausing on, because in this book it is a technical term that does not carry its everyday meaning.

In ordinary speech, to say that we expect something is to say that we anticipate it happening --- that it is the outcome we would bet on actually observing. That is not what \(\E[\,\cdot\,]\) means. An expected value is an \emph{average}, and nothing more. It is the center of a distribution. There is no promise anywhere in the mathematics that this center is a value any individual could ever have.

Recall the five outcomes above, \(7,\,9,\,10,\,11,\,13\), whose average is \(10\). There the expected value happened to be a value we might also observe. Change a single number, so that the outcomes are \(7,\,9,\,10,\,11,\,14\), and the average becomes \(10.2\). If \(Y\) is a count --- children in a household, prior hospital visits, questions answered correctly on a ten-item quiz --- then \(10.2\) is not an outcome that anyone can have. It is still the correct answer to the question we asked.

The point is sharpest when \(Y\) is binary. Let \(Y=1\) if a person is employed and \(Y=0\) otherwise, and suppose that among people with a particular value of \(X\), eighty percent are employed. Then
\[
\E[Y\mid X=x]=0.8 .
\]
Nobody in this population has \(Y=0.8\). Every single person has \(Y=0\) or \(Y=1\), and \(0.8\) is not merely unobserved but impossible. The conditional expectation is nonetheless exactly the quantity we want: when \(Y\) is binary, \(\E[Y\mid X=x]\) is the \emph{probability} of employment among people with that value of \(X\). We return to this connection when we discuss logistic regression later in this chapter.

So a conditional expectation should be read as a statement about the population, not a forecast about any one person. ``The expected income of someone with sixteen years of education is \$62{,}000'' does not predict that the next such person we meet will earn \$62{,}000; almost certainly they will not. It says that \$62{,}000 is the center of the income distribution among such people, and therefore that it is our best single guess --- best in a specific sense that Chapter~\ref{chap:foundations} makes precise, where we show that the mean is the constant prediction minimizing squared error, and that the same logic applied within groups makes each group's conditional mean the best squared-error prediction for that group. A good prediction, in this sense, is not one that comes true. It is one that is wrong by as little as possible, on average, across everyone it is applied to.
...
\index{Poisson regression}\index{negative binomial regression}\index{count outcomes!zeros in}\index{zero-inflated models}
...
0,1,2,3,\ldots
\]
and cannot be negative. Zero is an ordinary value here, not a special case: many people make no doctor visits in a year, and many neighborhoods record no crimes.
...
\]
The logarithmic link ensures that the predicted expected count remains positive. Because \(\lambda_i=\exp(X_i\beta)\) and the exponential function is never zero, every fitted value is strictly greater than zero, however small it may be.

That deserves a moment's attention, because two statements just made sound contradictory. The outcome \(Y_i\) can equal zero. The fitted value \(\lambda_i\) cannot. There is no contradiction, because these are different quantities. As explained earlier in this chapter, \(\lambda_i\) is a conditional mean, and a conditional mean need not be a value the outcome can actually take. A fitted \(\lambda_i\) of \(0.3\) does not predict that someone will make three-tenths of a doctor visit. It says that among people with these predictor values, the average is about a third of a visit per year --- most of them make none, and a few make one or more. The model assigns zero a probability, namely \(e^{-\lambda_i}\), which for \(\lambda_i=0.3\) is about \(0.74\); what it never does is predict a mean of exactly zero.

This also explains where the logarithm belongs. It is applied to the mean, not to the outcome. A natural first instinct is to regress \(\log(Y_i)\) on \(X_i\) directly, but \(\log(0)\) is undefined, so that approach breaks on exactly the observations that are most common in many count datasets. Discarding those observations, or adding a small constant to make the logarithm defined, changes the question being asked. Poisson regression sidesteps the problem by placing the link on \(\lambda_i\), which is always positive, and leaving the zeros in the data untouched.

When count data vary substantially more than the simple Poisson model allows, a negative binomial regression may be more appropriate. A common reason for that excess variability is an abundance of zeros: if a dataset contains many more zeros than a Poisson model with the fitted \(\lambda_i\) would produce, the model is misspecified, and specialized zero-inflated models exist for that situation.
...
A small \(p\)-value therefore says that the observed estimate is difficult to reconcile with the particular null hypothesis under the assumed statistical model. It does \emph{not} mean that the probability the null hypothesis is true is small. It also does not tell us whether the estimated quantity is substantively important.\footnote{\emph{Substantive importance} means importance in the real scientific, policy, practical, or decision context, rather than merely statistical detectability. A coefficient can be estimated very precisely and have a tiny \(p\)-value while still being too small to matter in practice.}
...
Suppose we are predicting wages from education, experience, occupation, region, and other variables. Is the relationship with experience linear? Does it flatten with age? Does education interact with occupation? Are relevant predictors missing? Does error variance differ across groups? Are observations independent, or are people nested inside common labor markets? There is rarely a known mathematical law that settles these questions.
...
\index{irreducible error}\index{reducible error}\index{data-generating process}\index{signal and noise}

Mean squared error tells us how far our predictions fall from the outcomes. It does not tell us \emph{why} they fall short. Some of the shortfall is our own doing and could be removed by a better model. Some of it could not be removed by any model at all. Separating these two sources is one of the most clarifying ideas in statistical learning, and it comes with standard nomenclature worth adopting.

Begin by writing the data-generating process in the form
\[
Y=f(X)+\varepsilon .
\]
It is important to see that this is a \emph{decomposition}, not an extra assumption. Define \(f(X)=\E[Y\mid X]\), which is simply the conditional expectation function from Chapter~\ref{chap:intro}, and define \(\varepsilon=Y-f(X)\) as whatever is left over. Written this way the equation is true by construction, and so is the property
\[
\E[\varepsilon\mid X]=0 ,
\]
because the average of \(Y\) around its own conditional average is zero. The systematic part \(f\) is often called the \emph{signal}, and \(\varepsilon\) the \emph{noise} or the \emph{disturbance}. Assumptions enter only when we say something further about \(\varepsilon\) --- that its variance is constant, say, or that it is normally distributed. Those are claims about the world. The decomposition itself is free.

Now suppose we have estimated some prediction function \(\widehat{f}\) from a training sample, and we use it to predict a new observation drawn at \(X=x\). The expected squared error of that prediction splits into two pieces:
\[
\underbrace{\E\big[(Y-\widehat{Y})^2 \,\big|\, X=x\big]}_{\text{expected squared prediction error}}
=
\underbrace{\big[f(x)-\widehat{f}(x)\big]^2}_{\text{reducible}}
\;+\;
\underbrace{\Var(\varepsilon\mid X=x)}_{\text{irreducible}} .
\]
The cross term vanishes precisely because \(\E[\varepsilon\mid X]=0\), so nothing has been assumed away to obtain this.

The two pieces have very different characters, and the names are the standard ones.

The first is the \emph{reducible error}. It measures how badly \(\widehat{f}\) approximates \(f\), and it is our responsibility. It can be reduced by choosing a better functional form, by including predictors that belong in the model, by collecting more data so the estimate is less noisy, or by using a more flexible method where flexibility is warranted. In principle it can be driven toward zero. Most of what statisticians and data scientists actually do is an attempt to shrink this term.

The second is the \emph{irreducible error}. It is the variability of \(Y\) among units that share the same value of \(X\), and no amount of modeling skill can remove it, because it is not a function of \(X\) at all. Even an analyst handed the true \(f\) --- perfect knowledge of the conditional expectation function --- would still make prediction errors of this size. Recall the five outcomes from Chapter~\ref{chap:intro}, \(7,\,9,\,10,\,11,\,13\), all sharing the same value of \(X\). Their spread around \(10\) is exactly this quantity. Knowing \(X\) more precisely does not help; every one of these units already has the same \(X\).

Where does irreducible error come from? Typically from three sources: variables that carry information about \(Y\) beyond what \(X\) contains, including variables nobody has measured or could measure; inherent randomness in the outcome itself; and measurement error in \(Y\). All three produce variation that the predictors are simply unable to speak to.

One qualification matters, and it is easy to miss. Irreducible error is irreducible \emph{given a particular set of predictors}. It is a property of the pair \((Y,X)\), not of \(Y\) alone. If we go out and measure a new variable and add it to \(X\), we have changed the conditioning set: \(f\) becomes a different function, and variation that was formerly irreducible may become reducible. This is one reason that acquiring better predictors is often more valuable than fitting a cleverer model to the predictors already in hand. What cannot be done is to squeeze that variation out by modeling alone, once the predictor set is fixed.

Finally, this decomposition explains something that will otherwise look strange in the next chapter. There we will meet two different intervals attached to a prediction. A confidence interval for the conditional mean describes uncertainty about where \(f\) lies, and that is reducible: it narrows as the sample grows, and shrinks toward zero width in the limit. A prediction interval for a single new observation must additionally absorb \(\Var(\varepsilon\mid X=x)\), and that term does not shrink at all. However much data we gather, the prediction interval retains a floor set by the irreducible error. That is the entire reason the two intervals differ in width, and why only one of them can be made arbitrarily narrow.
...
\section{A Coefficient's Confidence Interval Does Not Give a Predicted Value's Confidence Interval}
\index{variance-covariance matrix}\index{first difference}\index{confidence interval!predicted mean}\index{correlated coefficient estimates}

The previous section asked all three questions separately, using \texttt{confint()} for the coefficients and \texttt{predict()} for the fitted mean. A natural thought is that the second request is unnecessary --- that we could construct the interval for a predicted value ourselves out of the interval for the slope, by taking a \emph{first difference}: move \(\widehat{\beta}_1\) to each end of its confidence interval, multiply by \(x\), and read off the resulting range of predictions.

This does not work. It is worth seeing exactly why, because the reason is not a technicality: it is the single most important structural fact about estimated regression coefficients, namely that they are \emph{not} estimated independently of one another.

\subsection{The formula that connects the two kinds of uncertainty}

The fitted value at \(X=x\) is a linear combination of the estimated coefficients:
\[
\widehat{Y}(x)=\widehat{\beta}_0+\widehat{\beta}_1 x .
\]
The variance of any such combination is given by a standard result:\footnote{The object doing the work here is the \emph{variance-covariance matrix} of the estimated coefficients, written \(\widehat{\Var}(\widehat{\beta})\) and returned in R by \texttt{vcov(fit)}. It is a square, symmetric table with one row and one column per coefficient. Its diagonal entries are the variances of the individual estimates --- the squares of the standard errors printed in the regression output --- and its off-diagonal entries are the covariances between pairs of estimates. For any fixed vector \(c\), the estimator \(c'\widehat{\beta}\) has variance \(c'\,\widehat{\Var}(\widehat{\beta})\,c\). In OLS the matrix takes the familiar form \(\widehat{\sigma}^2(X'X)^{-1}\). This formula is the bridge between the two kinds of uncertainty in this chapter: uncertainty about a \emph{predicted value} is a function of uncertainty about the \emph{coefficients}, but it depends on the entire matrix, not on any single entry of it. That is precisely why a naive first difference built from one coefficient's standard error or confidence interval cannot recover it. The standard error printed next to \(\widehat{\beta}_1\) is one diagonal entry; the calculation needs the others too.}
\[
\Var\big(\widehat{Y}(x)\big)
=
\Var(\widehat{\beta}_0)
+
x^2\,\Var(\widehat{\beta}_1)
+
2x\,\operatorname{Cov}(\widehat{\beta}_0,\widehat{\beta}_1).
\]
The naive first-difference procedure uses only the middle term. It discards the uncertainty in the intercept entirely, and --- more damagingly --- it discards the covariance between the two estimates.

That covariance is not a small correction. For simple OLS with an intercept,
\[
\operatorname{Cov}(\widehat{\beta}_0,\widehat{\beta}_1)
=
-\,\overline{x}\,\Var(\widehat{\beta}_1),
\]
which is zero only in the special case \(\overline{x}=0\). The intuition is geometric, and worth carrying around: the fitted line always passes through the point \((\overline{x},\overline{y})\). It pivots about that point. So if a particular sample produces a slope estimate that is too steep, the intercept estimate must come down to compensate, or the line would no longer pass through the sample's center of mass. The two estimation errors are yoked together. When \(\overline{x}>0\), as it is for most predictors measured on a positive scale, they are \emph{negatively} correlated, often very strongly.

\subsection{An empirical example: the \texttt{cars} data with an intercept}
...
# The off-diagonal entry is large and negative. As a correlation:
cov2cor(vcov(fit))

# It equals -xbar * Var(beta1hat):
-mean(cars$speed) * vcov(fit)["speed", "speed"]

# The CORRECT interval for the conditional mean at speed = 15
predict(fit, newdata = data.frame(speed = 15), interval = "confidence")

# The NAIVE interval: hold the intercept at its estimate and swing
# the slope across the two ends of its confidence interval.
coef(fit)[1] + confint(fit)["speed", ] * 15
\end{lstlisting}

The estimated coefficients are \(\widehat{\beta}_0=-17.58\) and \(\widehat{\beta}_1=3.93\), and the variance-covariance matrix is
\[
\begin{pmatrix}
45.677 & -2.659\\
-2.659 & \phantom{-}0.173
\end{pmatrix},
\]
corresponding to a correlation between the two estimates of \(-0.947\). These estimates are very nearly redundant with one another, and the mean speed of \(15.4\) is exactly the multiplier that produces the off-diagonal entry.

The two intervals for the conditional mean at \(\text{speed}=15\) are
\[
\text{correct: }[37.02,\;45.79],
\qquad
\text{naive: }[28.88,\;53.94].
\]
The naive interval is roughly \emph{three times too wide}. It is instructive to see where that comes from. The three terms in the variance formula are
\[
\underbrace{45.677}_{\Var(\widehat{\beta}_0)}
+
\underbrace{38.846}_{x^2\Var(\widehat{\beta}_1)}
+
\underbrace{(-79.765)}_{2x\operatorname{Cov}}
=
4.758,
\]
so the correct standard error is \(\sqrt{4.758}=2.18\), while the naive calculation reports \(15\times\sqrt{0.173}=6.23\).

Notice what the covariance term is doing. It is negative, and larger in magnitude than either variance on its own. Almost all of the apparent uncertainty in the two coefficients \emph{cancels} when they are combined into a prediction, because the errors move in opposite directions. Ignoring the covariance therefore does not produce a slightly imperfect answer; it produces an answer that is wrong by a factor of three, and in this case wrong in the direction of being far too conservative.

That last detail deserves emphasis, because the mistake is usually described as though omitting a source of uncertainty must make an interval too narrow. Here the opposite happens. Whether the naive interval is too wide or too narrow depends on the sign of the covariance term and on where \(x\) sits relative to \(\overline{x}\). There is no safe direction in which to be wrong, and no rule of thumb that rescues the shortcut.

\subsection{The two widths are not even the same shape}

There is a sharper way to see that the shortcut cannot work in general, and it does not depend on any particular dataset. Compare the two widths as \emph{functions of \(x\)}.

Substituting the endpoints of the coefficient's interval into the prediction equation produces predictions that differ by
\[
\big(\text{upper}-\text{lower}\big)\times x
=
2t\,\mathrm{SE}(\widehat{\beta}_1)\,x ,
\]
which is a straight line through the origin. The correct width, written in its standard form, is
\[
2t\,\widehat{\sigma}\sqrt{\frac{1}{n}+\frac{(x-\overline{x})^2}{S_{xx}}},
\]
which is a hyperbola with a strictly positive minimum at \(x=\overline{x}\). A line through the origin and a curve that never reaches zero are simply different shapes. They may cross, but they cannot coincide.

The clearest case is \(x=0\). There the naive construction gives a width of exactly zero: both endpoints of the coefficient interval, multiplied by zero, return the same prediction \(\widehat{\beta}_0\). Taken at face value, the procedure claims we know \(E[Y\mid X=0]\) with perfect certainty. But \(E[Y\mid X=0]\) is precisely the intercept, and its standard error is printed in the regression output. The shortcut has discarded the one quantity that mattered.

The \texttt{cars} data show the full pattern. Widths of the three intervals, at a range of values of \(x\):

\begin{center}
\begin{tabular}{rrrrr}
\toprule
\(x\) & Naive width & Correct CI width & Prediction interval & Naive \(\div\) CI\\
\midrule
0    & 0.00   & 27.18  & 67.55 & 0.00\\
5    & 8.35   & 19.45  & 64.83 & 0.43\\
8.59 & 14.35  & 14.35  & 63.66 & 1.00\\
15.4 \((=\overline{x})\) & 25.73 & 8.75 & 62.46 & 2.94\\
20   & 33.42  & 11.64  & 62.93 & 2.87\\
25   & 41.77  & 18.27  & 64.49 & 2.29\\
100  & 167.09 & 141.63 & 154.54 & 1.18\\
\bottomrule
\end{tabular}
\end{center}

The shortcut is far too narrow for small \(x\), exactly right at a single point, and nearly three times too wide near the middle of the data --- which is, unhelpfully, where most predictions are actually made. The crossing point here is \(x=8.59\), obtained by solving \(\Var(\widehat{\beta}_0)+2x\operatorname{Cov}(\widehat{\beta}_0,\widehat{\beta}_1)=0\), which gives \(x^\ast=\overline{x^2}/(2\overline{x})\). It falls inside the observed range of speeds, \(4\) to \(25\). So the procedure is not even reliably conservative: an analyst cannot claim to be erring on the side of caution, because the direction of the error changes within the span of the data.

The ratio does approach one as \(x\) grows large, since the slope term eventually dominates the variance. That is cold comfort, because the region where the approximation improves is the extrapolation region, where the interval is least trustworthy for other reasons entirely.

The final column of the table settles the remaining question. The naive width is nowhere near the prediction interval either. Near the data the prediction interval is roughly \(62\) to \(68\) units wide, while the naive construction ranges from \(8\) to \(42\). This is unsurprising once stated: the prediction interval must include the irreducible error \(\widehat{\sigma}^2\) of Chapter~\ref{chap:foundations}, and the naive calculation contains no information about the spread of \(Y\) around the line at all. It is built entirely from coefficient uncertainty. Nothing in it could ever produce a prediction interval.

\subsection{The same data with no intercept}

Now fit a line through the origin, so that the model has exactly one coefficient:

\begin{lstlisting}
fit0 <- lm(dist ~ speed - 1, data = cars)

# A 1x1 variance-covariance matrix: there is no covariance to ignore.
vcov(fit0)

predict(fit0, newdata = data.frame(speed = 15), interval = "confidence")

confint(fit0)["speed", ] * 15
\end{lstlisting}

Here the two approaches agree exactly. Both return \([39.38,\;47.90]\), to every digit R will print. The reason is immediate from the formula: with a single coefficient,
\[
\widehat{Y}(x)=\widehat{\beta}_1x,
\qquad
\Var\big(\widehat{Y}(x)\big)=x^2\,\Var(\widehat{\beta}_1),
\]
and the variance-covariance matrix has collapsed to a single number. There is no intercept to carry its own uncertainty and no off-diagonal entry to discard, so rescaling the coefficient's interval is not an approximation --- it is the correct calculation.

This is the exception that identifies the rule, and the rule is worth stating plainly.

\begin{quote}
Rescaling one coefficient's confidence interval is valid exactly when the quantity of interest depends on exactly one estimated coefficient. Whenever it depends on two or more, their covariances enter, and the shortcut fails.
\end{quote}

\subsection{A different question: comparing two predictor values}

The phrase \emph{first difference} is used in two ways, and it is worth separating them, because one of the two constructions is legitimate and the other is the mistake this section is about.

Everything above concerns differencing across the \emph{endpoints of a coefficient's confidence interval}, at a single value of \(x\), in the hope of recovering an interval for a predicted value. That does not work, for the reasons given.

A genuinely different quantity is the difference between predictions at \emph{two values of the predictor}, with the coefficients held at their estimates. This is the sense the term usually carries in applied work, and here the news is better. Suppose we want to know how much higher the predicted stopping distance is at \(\text{speed}=20\) than at \(\text{speed}=15\). In the simple model with an intercept,
\[
\widehat{Y}(20)-\widehat{Y}(15)
=
\widehat{\beta}_1(20-15),
\]
and the intercept has cancelled. The quantity now depends on one coefficient only, so rescaling its interval by \(5\) is legitimate. In the \texttt{cars} data both routes give a standard error of \(2.078\).

But this survives only because the model is so simple, and because the intercept happened to drop out. Add a quadratic term and the same difference becomes
\[
\widehat{\beta}_1(20-15)+\widehat{\beta}_2(20^2-15^2),
\]
which loads on two estimated coefficients whose correlation in these data is \(-0.979\). The correct standard error for that difference is \(2.59\); using the \texttt{speed} coefficient alone gives \(10.17\), too large by a factor of four. The same failure returns with interactions, with polynomial terms, with splines, and in any multiple regression where changing one predictor changes more than one term in the fitted equation.

So the trustworthy habit is not to memorize which special cases are safe. It is to let \texttt{predict()} do the calculation, which uses the whole variance-covariance matrix automatically, or --- when the quantity of interest is some other combination that \texttt{predict()} does not offer --- to build the vector \(c\) and compute \(c'\,\texttt{vcov(fit)}\,c\) directly.
...
\section{What a Confidence Interval Still Means When a Confounder Is Omitted}
\index{omitted variable bias}\index{population linear projection}\index{estimand}\index{confidence interval!coefficient}

The previous section said that a coefficient is not automatically a causal effect. It is worth working out what happens to the \emph{confidence interval} in that situation, because the answer is more interesting than simply declaring the analysis broken.

Suppose the outcome is generated as
\[
Y=\alpha_0+\alpha_1X+\alpha_2Z+u,
\qquad
\E[u\mid X,Z]=0,
\]
and suppose that, under a design or set of assumptions we are willing to defend, \(\alpha_1\) is the causal effect of \(X\). Now suppose we do not have \(Z\). Perhaps it was never measured. We fit the short regression
\[
Y=\beta_0+\beta_1X+\varepsilon
\]
and R obligingly reports \(\widehat{\beta}_1\), a standard error, and a \(95\%\) confidence interval.

Standard algebra tells us what \(\beta_1\) actually is. Let \(\delta_1\) be the slope from the population regression of \(Z\) on \(X\). Then
\[
\beta_1=\alpha_1+\alpha_2\delta_1 .
\]
The second term is the classic \emph{omitted variable bias}. It is nonzero whenever \(Z\) both affects \(Y\) \((\alpha_2\neq 0)\) and is related to \(X\) \((\delta_1\neq 0)\) --- which is exactly what it means for \(Z\) to be a confounder. So \(\beta_1\neq\alpha_1\), and our estimator is not aimed at the causal effect.

\subsection{The interval is not broken; it is aimed elsewhere}

Here is the key point. The confidence interval is not malfunctioning. It is doing its job accurately --- for a different target than the one we may have had in mind.

The quantity \(\beta_1\) is not a fiction, and it is not merely ``the biased version of \(\alpha_1\).'' It is a well-defined feature of the population in its own right: the slope of the best linear predictor of \(Y\) using \(X\) alone, sometimes called the \emph{population linear projection} of \(Y\) on \(X\). This object exists whether or not any linear model is ``true,'' because it is defined by a minimization --- the line that minimizes expected squared error in the population --- rather than by a claim about how the world works. OLS remains a consistent estimator of it under misspecification, and heteroskedasticity-robust standard errors deliver asymptotically valid inference for it.\footnote{This is why robust standard errors are the conventional default in this setting. Once we admit that the conditional mean is not literally linear, the homoskedastic textbook variance formula loses its justification, whereas the sandwich formula remains valid for the projection coefficient under mild conditions.} Under those conditions, the reported interval covers \(\beta_1\) about \(95\%\) of the time, exactly as advertised.

And \(\beta_1\) often answers a question someone genuinely wants answered. In words, it says: among units that differ in \(X\) by one unit, how much does average \(Y\) differ, \emph{without} adjusting for \(Z\)? That is a real and frequently important quantity. Unadjusted differences in wages across groups, raw disparities in health outcomes, the predictive value of a variable that a decision-maker will actually observe in practice, a descriptive summary of an association, a forecasting rule --- all of these are legitimately about \(\beta_1\), not \(\alpha_1\). A hospital deciding whom to screen may care what an available predictor predicts, not what it causes. Reporting a confidence interval for such a quantity is honest and useful work.

\subsection{As an interval for the causal effect, it is worse than useless}

The same interval, relabeled as a statement about \(\alpha_1\), becomes actively misleading. Not merely uninformative --- misleading, and in a way that gets worse rather than better as the study improves.

The reason is that a confidence interval quantifies \emph{sampling variability only}. It answers the question: how much would this estimate bounce around across repeated samples? It contains no information whatsoever about bias from an omitted confounder. Those are different failure modes, and the standard error is blind to the second one.

The consequence is uncomfortable. As \(n\) grows, the interval's width shrinks toward zero, but the bias \(\alpha_2\delta_1\) does not shrink at all --- it is a feature of the population, not of the sample. So the interval closes in tightly around \(\beta_1\), a number that is not \(\alpha_1\). Its coverage of the causal effect therefore falls toward \emph{zero}: in a large enough sample, a nominal \(95\%\) interval will exclude the true causal effect essentially always. More data buys us more confident wrongness, reported with a smaller standard error and a more impressive \(p\)-value.

The lesson is that precision is not accuracy. A narrow interval means the estimate is stable, not that it is correct. Nothing in the regression output distinguishes the two cases, because the arithmetic is identical either way. What changes is only the label the analyst attaches to the result.

\subsection{Coefficients are indexed to a specification}

The discipline this suggests is to stop speaking of ``the effect of \(X\)'' and start naming the estimand precisely. A regression coefficient is never simply a property of \(X\) and \(Y\). It is a property of a particular prediction rule, fitted to a particular set of variables, within a particular functional form. It is a coefficient of \emph{that} machine.

Change any of those and the coefficient changes meaning, not merely value:

\begin{itemize}[leftmargin=2em]
\item Add \(Z\) to the regression and the estimand becomes the projection coefficient holding \(Z\) fixed --- a different population quantity, which is why coefficients move when predictors are added, as we saw earlier in this chapter.
\item Add a quadratic term or an interaction and there may no longer be a single coefficient describing the relationship at all.
\item Fit LOESS instead and there is no coefficient to report, though the conditional mean being estimated is the same object.
\end{itemize}

None of these is more correct in the abstract. Each targets a different estimand, and the analyst's obligation is to say which one is being reported.

That obligation is what makes the difference between defensible and indefensible practice here, and the gap between them is entirely verbal. Writing ``\(95\%\) confidence interval for the unadjusted association between \(X\) and \(Y\)'' is accurate, useful, and the interval genuinely supports it. Writing ``\(95\%\) confidence interval for the effect of \(X\) on \(Y\)'' is not supported, and the numbers printed on the page are exactly the same in both cases. The statistics cannot tell these apart. Only the analyst can.

This is also why the causal work in Chapter~\ref{chap:causal_bridge} is done by design --- randomization, overlap, and comparability --- rather than by tightening a confidence interval. Sampling uncertainty and identification are separate problems, and the confidence interval addresses only the first.
...
Notice the final point carefully. If \(X=0\) is outside the observed data or is substantively meaningless, the coefficient on the binary variable can become awkward to interpret.
...
The star expands to \texttt{x}, \texttt{d}, and their interaction.\footnote{Terms that appear on their own alongside an interaction are conventionally called \emph{main effects}, a name inherited from the analysis of variance. That name is worth recognizing, since it is standard in textbooks and in R's own documentation, but this book avoids it: the quantity is a coefficient in a particular specification, not an effect that a research design has identified.}

\section{Empirical Example: An Additive Model Imposes Parallel Lines}
\index{mtcars data set@\texttt{mtcars} data set}\index{interaction!absence of}

The restriction is easiest to see in a case where it does no harm. Using the built-in \texttt{mtcars} data, predict fuel economy from gross horsepower (\texttt{hp}) and transmission type (\texttt{am}), with no interaction term:

\begin{lstlisting}
fit_additive <- lm(mpg ~ hp + am, data = mtcars)
...
The coefficient on \texttt{hp} is \(-0.0589\) in both equations, because the specification contains only one such coefficient. The two fitted lines are therefore parallel by construction, separated everywhere by the same vertical distance of \(5.277\) miles per gallon. Figure~\ref{fig:additive_mtcars} shows them.
...
(62,24.4) (95,22.8) (97,21.5) (105,18.1) (110,21.4) (123,19.2)
(123,17.8) (150,15.5) (150,15.2) (175,18.7) (175,19.2) (180,16.4)
(180,17.3) (180,15.2) (205,10.4) (215,10.4) (230,14.7) (245,14.3)
(245,13.3)
};
\addlegendentry{Automatic observations}

\addplot[only marks,mark=*,mark size=1.8pt] coordinates {
(52,30.4) (65,33.9) (66,32.4) (66,27.3) (91,26.0) (93,22.8)
(109,21.4) (110,21.0) (110,21.0) (113,30.4) (175,19.7) (264,15.8)
(335,15.0)
};
\addlegendentry{Manual observations}

\addplot[domain=62:245,samples=2,very thick] {26.584914 - 0.0588878*x};
\addlegendentry{Automatic fitted line}

\addplot[domain=52:335,samples=2,very thick,dashed] {31.861999 - 0.0588878*x};
\addlegendentry{Manual fitted line}
\end{axis}
\end{tikzpicture}
\caption{With no interaction term, the additive model forces one common slope on \texttt{hp}. The two fitted lines can differ only by a constant vertical shift, here estimated at \(5.277\) miles per gallon with a standard error of \(1.08\). The lines are drawn across each group's observed horsepower range.}
\label{fig:additive_mtcars}
\end{figure}

Here the restriction is defensible. Allowing the slopes to differ,

\begin{lstlisting}
fit_interaction_hp <- lm(mpg ~ hp * am, data = mtcars)
coef(fit_interaction_hp)["hp:am"]
\end{lstlisting}

returns an interaction estimate of about \(0.0004\), with a standard error of about \(0.016\). Two separate things are worth noting. First, the restriction is cheap in this sample: dropping the interaction moves no fitted value by more than \(0.03\) miles per gallon and leaves \(R^2\) unchanged at \(0.782\). Second, the interaction is estimated imprecisely---its standard error is roughly forty times the estimate itself---so the defensible statement is that these data give no reason to think the slopes differ, not that the slopes are known to be equal. With thirty-two cars, a moderate difference in slopes would not be detectable. Imposing a common slope is inexpensive here, but it remains a restriction rather than a finding. The next section shows a case where the same restriction is not cheap at all.

The base-R version of the graph:
...
# Each group's line is drawn only over that group's observed support
auto_hp <- seq(
...
\section{With an Interaction, the Fitted Slope Depends on the Other Variable}

Once an interaction is present, there is generally no single coefficient that describes the fitted relationship between \(X\) and \(Y\) everywhere in the model.
...
Figure~\ref{fig:gap_interpolation} shows what the two models do across the gap.
...
\draw[fill=black!10,draw=none] (axis cs:4,3) rectangle (axis cs:7,24);
\draw[densely dotted,thick] (axis cs:4,3) -- (axis cs:4,24);
\draw[densely dotted,thick] (axis cs:7,3) -- (axis cs:7,24);

\addplot[only marks,mark=*,mark size=1.5pt] coordinates {
(1.026,5.81) (1.050,5.62) (1.269,4.81) (1.405,6.23) (1.697,6.84) (1.842,7.78)
(1.868,7.59) (1.935,7.96) (1.937,8.51) (2.023,8.64) (2.176,9.17) (2.268,9.08)
(2.338,8.86) (2.406,9.56) (2.414,8.72) (2.452,9.31) (2.476,8.92) (2.532,9.11)
(2.653,8.99) (2.677,9.89) (2.693,9.67) (2.988,10.62) (3.008,11.85) (3.051,10.55)
(3.205,12.16) (3.222,10.97) (3.364,11.58) (3.387,12.12) (3.400,12.84) (3.441,12.53)
(3.460,12.31) (3.529,12.42) (3.623,12.93) (3.839,13.14) (3.974,14.27)
(7.038,20.73) (7.055,20.13) (7.125,21.93) (7.204,20.93) (7.211,21.57) (7.256,21.04)
(7.456,20.29) (7.531,20.91) (7.544,20.59) (7.692,20.39) (7.722,19.80) (7.751,19.99)
(7.834,19.87) (8.007,20.32) (8.057,19.72) (8.410,20.68) (8.624,19.36) (8.823,19.31)
(8.828,17.76) (8.921,18.64) (8.963,19.00) (8.995,18.94) (9.102,19.26) (9.114,18.23)
(9.160,18.52) (9.355,17.81) (9.449,18.44) (9.516,18.58) (9.524,18.77) (9.546,17.78)
(9.776,18.18) (9.807,18.12) (9.872,17.59) (9.985,17.02) (9.989,17.88)
};
\addlegendentry{Training observations}

\addplot[domain=1:10,samples=2,very thick]{6.09039 + 1.52535*x};
\addlegendentry{Linear fit}

\addplot[domain=1:10,samples=120,very thick,dashed]{-1.82436 + 5.55963*x - 0.35848*x^2};
\addlegendentry{Quadratic fit}

\addplot[only marks,mark=o,mark size=3.2pt,very thick] coordinates {(5.5,14.48)};
\addlegendentry{Linear prediction at \(X=5.5\)}

\addplot[only marks,mark=triangle,mark size=3.6pt,very thick] coordinates {(5.5,17.91)};
\addlegendentry{Quadratic prediction at \(X=5.5\)}
\end{axis}
\end{tikzpicture}
\caption{Two models fitted to the same data, which contain no observations between \(X=4\) and \(X=7\) (shaded). Both fit the observed regions closely and both return a prediction at \(X=5.5\), but they disagree by about \(3.4\) units. Nothing in the data adjudicates between them, because nothing was observed there.}
\label{fig:gap_interpolation}
\end{figure}
...
That advice is neither new nor controversial. It appears in introductory texts, in the regression-diagnostics literature, and in essentially every treatment of causal inference written in the last forty years. Whether it is routinely \emph{followed} is a different question, and our impression is that it is not. Published regression tables report coefficients, standard errors, and \(R^2\); they rarely report anything about where the predictor data are dense and where they are thin. Referees ask whether a coefficient is statistically significant far more often than they ask whether the comparison it encodes is supported by observations. The result is a practice that is universally endorsed and frequently skipped --- and skipped silently, because the omission leaves no trace in the output. A regression fitted where the data are thick and a regression fitted where the data are almost absent print the same table.
...
This difficulty has a name, and it gets rapidly worse as predictors accumulate. As the number of predictors grows, the volume of the space they span grows exponentially, so any fixed sample becomes sparse within it. Statisticians call this the \emph{curse of dimensionality}, a phrase due to Richard Bellman.\footnote{Richard E. Bellman, \emph{Dynamic Programming} (Princeton, NJ: Princeton University Press, 1957), where the phrase is introduced in the preface; he returns to it in \emph{Adaptive Control Processes: A Guided Tour} (Princeton University Press, 1961). Bellman's own concern was computational --- the cost of dynamic programming grows exponentially as state variables are added --- but the phrase has long since been borrowed for the statistical problem described here, where what grows exponentially is the amount of data needed to populate the space.}\index{curse of dimensionality}

The arithmetic is worth seeing. Suppose we want enough data that no point in the predictor space is farther than \(0.1\) from an observation. On a single predictor scaled to the unit interval, about \(10\) observations suffice. On two predictors --- the unit square --- it takes about \(100\). On three, about \(1{,}000\). On ten predictors it takes \(10^{10}\), ten billion observations, and on twenty it takes more observations than any study will ever collect. The requirement multiplies by ten with each predictor added.

Why is this a \emph{curse} rather than merely an inconvenience? Because adding a predictor feels like acquiring information while it simultaneously thins the information already in hand. The sample size does not change; the space those observations must cover expands. Neighbors that were close in two dimensions become distant in ten, and in high dimensions almost every observation is far from almost every other one. The local evidence that disciplines a fitted function --- observations near the point where a prediction is wanted --- quietly disappears, and the functional form silently takes over the work. The intuition built on a single predictor, \emph{my data run from 2 to 10, so a prediction at 6 is an interpolation}, therefore becomes steadily more misleading as predictors are added. With enough of them, nearly every prediction is an extrapolation in the sense that actually matters, however comfortably each individual variable sits inside its own observed range.


This is why \emph{support} is a better concept than the one-dimensional min--max range. In several dimensions, support concerns the predictor combinations that are actually represented by observations.\footnote{The definitive treatment is Gary King and Langche Zeng, ``The Dangers of Extreme Counterfactuals,'' \emph{Political Analysis} 14 (2006): 131--159, available at \url{https://gking.harvard.edu/files/counterft.pdf}. King and Zeng make the idea precise using the \emph{convex hull} of the observed predictor data --- the smallest convex set containing all the observations. A counterfactual inside the hull requires interpolation; one outside requires extrapolation. They prove that inferences farther from the data permit more model dependence, and they show that in real applications counterfactuals routinely fall outside the hull even when every variable is individually within its observed range. They also note that directly comparing multivariate predictor distributions is ``essentially impossible without stringent assumptions'' once there are more than a few variables --- the curse of dimensionality again.}
...
\subsection{When the specification decides what can be found}
\index{model dependence}\index{functional form!and discoverability}

The support question --- are there observations near the comparison I want to make? --- has a close cousin. Even where the data are plentiful, the specification can rule out conclusions in advance, so that a pattern present in the data cannot appear in the results no matter how strongly the data express it.

The sharpest statement of this we know comes from a methodological exchange about predicting international conflict. Beck, King, and Zeng were responding to a critique of their neural-network models of war, in which the critics had substituted an ordinary logistic regression. Examining what that specification could and could not represent, the authors wrote that rival hypotheses

\begin{quote}
were impossible to uncover with the logit model \ldots specified, \emph{no matter how loudly the patterns in the data screamed}.\footnote{Nathaniel Beck, Gary King, and Langche Zeng, ``Theory and Evidence in International Conflict: A Response to de Marchi, Gelpi, and Grynaviski,'' \emph{American Political Science Review} 98, no.~2 (May 2004): 379--389, at 387. Available at \url{https://gking.harvard.edu/files/toe-resp.pdf}.}
\end{quote}

The phrase repays unpacking, because it is not rhetorical excess. The logit specification in question entered the two countries' democracy scores through their product and squared product. That is an algebraic restriction with a substantive consequence: it forces the fitted probability of conflict for two autocracies, at \((-10,-10)\), to resemble the probability for a mixed pair at \((-10,10)\) or \((10,-10)\). A researcher using that specification could discover a democratic peace, because the model can place low conflict probability where both scores are high. But an \emph{autocratic} peace --- low conflict probability where both scores are low --- was not a hypothesis the model could express. Fitting it more carefully, or with more data, would not have helped. The restriction was in the equation, not in the evidence.

When the same data were fitted with a model that nests the logit as a special case, two separate low-conflict regions appeared, one for mutual democracies and one for mutual autocracies, with a ridge of high conflict probability between them. The second region was not a subtlety: across it the fitted probability fell by roughly thirty percentage points.

The general lesson belongs beside the support question rather than replacing it. Both are ways a regression can return a confident number that the data did not license. Weak support means there were no observations near the comparison being made. A restrictive functional form means the answer was constrained before the data were consulted at all. In the first case the data are silent; in the second, as Beck, King, and Zeng put it, the data may be screaming and the model unable to hear it.
...
Prediction error also decomposes into two parts with very different characters. Writing the data-generating process as \(Y=f(X)+\varepsilon\), with \(f(X)=E[Y\mid X]\) and \(\varepsilon=Y-f(X)\), the expected squared error of a prediction at \(X=x\) is \([f(x)-\widehat{f}(x)]^2+\Var(\varepsilon\mid X=x)\). The first term is \emph{reducible error}: it reflects how well the estimated prediction function approximates the conditional expectation function, and better functional form, more informative predictors, and more data can shrink it. The second is \emph{irreducible error}: the variability of \(Y\) among units sharing the same \(X\), which no model can remove because it is not a function of \(X\). Irreducible error is defined relative to a given predictor set, so measuring genuinely new predictors can convert some of it into reducible error, whereas modeling alone cannot. This decomposition is also why a prediction interval cannot be narrowed indefinitely: it must absorb the irreducible term, which does not shrink with sample size, while a confidence interval for the conditional mean does.
...
These interval types are also not interconvertible by hand. A common temptation is to build an interval for a predicted value out of an interval for a coefficient, by taking a first difference: swinging \(\widehat{\beta}_1\) to each end of its confidence interval and multiplying by \(x\). This fails, because estimated coefficients are not estimated independently. The variance of a fitted value is \(\Var(\widehat{\beta}_0)+x^2\Var(\widehat{\beta}_1)+2x\operatorname{Cov}(\widehat{\beta}_0,\widehat{\beta}_1)\), and the naive procedure uses only the middle term. In simple OLS the covariance equals \(-\overline{x}\Var(\widehat{\beta}_1)\), which vanishes only when \(\overline{x}=0\); geometrically, the fitted line pivots about \((\overline{x},\overline{y})\), so a slope that is too steep forces an intercept that is too low. More generally, the variance of any linear combination \(c'\widehat{\beta}\) is \(c'\widehat{\Var}(\widehat{\beta})c\), which depends on the entire variance-covariance matrix and not on any single standard error. Rescaling one coefficient's interval is valid exactly when the quantity of interest depends on exactly one estimated coefficient---as in a regression through the origin, or a first difference in a model whose intercept cancels---and fails whenever two or more coefficients are involved.
...
A confidence interval can nevertheless remain useful when a model is known to be misspecified, provided its target is named honestly. If a confounder \(Z\) is omitted, the short-regression coefficient is \(\beta_1=\alpha_1+\alpha_2\delta_1\) rather than the causal effect \(\alpha_1\). The interval is not thereby broken: \(\beta_1\) is a well-defined population quantity---the coefficient of the population linear projection of \(Y\) on \(X\) alone---and OLS with robust standard errors supports valid inference about it. Reported as an unadjusted association, a descriptive summary, or the predictive value of an available variable, the interval does real work. Reported as a causal effect, it is worse than useless, because a confidence interval quantifies sampling variability only and is blind to bias from omitted confounders. As the sample grows the interval narrows while the bias does not, so its coverage of the causal effect falls toward zero: more data buys more confident wrongness. Precision is not accuracy, and the arithmetic is identical under either label. A coefficient is therefore always a property of a particular prediction rule fitted to a particular set of variables in a particular functional form---an estimand indexed to a specification---and never simply ``the effect of \(X\).''
...
\item[Data-generating process (DGP)] The underlying process or probability distribution that produces the variables we observe. Writing \(Y=f(X)+\varepsilon\) with \(f(X)=E[Y\mid X]\) is a decomposition of the DGP that is true by construction; assumptions enter only through further claims about \(\varepsilon\).\index{data-generating process}

\item[Expected value] The average, or center, of a distribution, written \(E[\,\cdot\,]\). Despite the everyday connotation of the word, an expected value is not the outcome we anticipate observing, and it need not be a value any individual could have: the expected value of a binary outcome is a probability strictly between zero and one, which no individual observation can equal.\index{expected value!meaning of the term}
...
\item[First difference] The change in a predicted or fitted quantity produced by moving a predictor from one value to another, such as \(\widehat{Y}(x_2)-\widehat{Y}(x_1)\), with the coefficients held at their estimates. Its uncertainty generally involves several estimated coefficients and therefore their covariances; it can be obtained by rescaling a single coefficient's confidence interval only when the difference depends on exactly one coefficient. The term is sometimes used instead for a different and invalid construction: differencing the two predictions obtained by substituting the endpoints of one coefficient's confidence interval, at a single value of \(x\), in the hope of recovering an interval for a predicted value. That quantity is a straight line in \(x\), whereas the correct width is a hyperbola minimized at \(\overline{x}\); the two are not the same interval and generally not the same width.\index{first difference}
...
\item[Irreducible error] The variability of \(Y\) among units sharing the same value of \(X\), equal to \(\Var(\varepsilon\mid X=x)\). No model can remove it, because it is not a function of \(X\). It is defined relative to a given predictor set: measuring additional predictors changes the conditioning set and can convert irreducible error into reducible error.\index{irreducible error}
...
\item[Omitted variable bias] The discrepancy between a short regression's coefficient and a causal parameter that arises when a confounder is excluded. If \(Y=\alpha_0+\alpha_1X+\alpha_2Z+u\) and \(Z\) is omitted, the short-regression coefficient is \(\beta_1=\alpha_1+\alpha_2\delta_1\), where \(\delta_1\) is the population slope of \(Z\) on \(X\). The bias term does not shrink with sample size.\index{omitted variable bias}
...
\item[Population linear projection] The best linear predictor of \(Y\) from a given set of predictors, defined as the linear function minimizing expected squared error in the population. It exists whether or not the conditional mean is truly linear, and it is the estimand that OLS consistently estimates under misspecification. A coefficient is therefore always indexed to a specification, not a property of \(X\) and \(Y\) alone.\index{population linear projection}
...
\item[Reducible error] The component of expected squared prediction error attributable to the gap between the estimated prediction function and the true conditional expectation function, \([f(x)-\widehat{f}(x)]^2\). Better functional form, more informative predictors, and more data can shrink it.\index{reducible error}

\item[Residual] The difference between an observed outcome and its fitted value, \(e_i=Y_i-\widehat{Y}_i\).\index{residual}

\item[Standard error] The estimated standard deviation of an estimator's sampling distribution. The standard errors printed beside regression coefficients are the square roots of the diagonal entries of the variance-covariance matrix, and they say nothing on their own about how the estimates covary.\index{standard error}
...
\item[Variance-covariance matrix] The square, symmetric table \(\widehat{\Var}(\widehat{\beta})\), returned by \texttt{vcov()} in R, whose diagonal holds the variances of the estimated coefficients and whose off-diagonal entries hold their covariances. For any fixed vector \(c\), the combination \(c'\widehat{\beta}\) has variance \(c'\,\widehat{\Var}(\widehat{\beta})\,c\). This formula connects uncertainty about coefficients to uncertainty about predicted values, and it depends on the whole matrix rather than on any single entry.\index{variance-covariance matrix}
